...
public class GameMap {
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        for (Entity e : getEntities(entity.getPosition())) {
            if (e != entity && e instanceof OverLap overlappable) {
    ...
    public Position dijkstraPathFind(Position src, Position dest, Entity entity) {
        return DijkstraPathFinder.findNext(this, src, dest, entity);
    }
    ...
    public boolean hasPortal(Position pos) {
        return nodes.containsKey(pos)
        && nodes.get(pos).getEntities().stream().anyMatch(Portal.class::isInstance);
    }

    public Portal getPortal(Position pos) {
        return nodes.get(pos).getEntities().stream()
    ...
    public int getTileWeight(Position pos) {
        return nodes.containsKey(pos) ? nodes.get(pos).getWeight() : 1;
    }
}
